...
func (h *HttpHandlerImpl) Router(router *fiber.App) {
	...
	api.Route("/product", func(router fiber.Router) {
		...
		router.Post("/create", middlewareJwt.JwtVerifyToken, middleware.HandleSingleFileImg, handler.CreateProduct)
		router.Put("/:id/update", middlewareJwt.JwtVerifyToken, middleware.HandleSingleFileImg, handler.UpdateProduct)
		router.Delete("/:id/delete", middlewareJwt.JwtVerifyToken, handler.DeleteProduct)
	})
